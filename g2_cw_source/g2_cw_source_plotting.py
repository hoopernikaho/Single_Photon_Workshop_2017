...
if __name__ == '__main__':
	...
	plt.ylabel('counts')
	plt.savefig('g2_cw_binsize_{:.0f}ns_range_{:.0f}ns.pdf'.format(np.diff(bins)[0],lims[1]), bbox_inches='tight')
	...
	plt.xlabel('pulse separation (us)')
	plt.ylabel('counts')
	plt.savefig('g2_cw_binsize_{:.0f}ns_range_{:.0f}ns.svg'.format(np.diff(bins)[0],lims[1]), bbox_inches='tight')
	plt.show()
